...
class BacktestingEngine:
    ...
    def __init__(self):
        ...
        self.trade_count = 0
        self.trades = {}
        self.xls_trade_count = 0    #  xls 添加
        self.xls_pos = []
        self.xls_capital = self.capital
        self.logs = []
    ...
    def run_backtesting(self):
        ...
        self.process_account_event() # xls 添加 put capital to strategy
    ...
    # xls 添加 回测账户信息 实盘的时候是通过 cta_strategy/engine.py  process_account_event推送account到策略中========
    # xls 实盘账户信息与此处不一样。需要修strategy 中的on_account

    def process_account_event(self):
        if self.xls_pos:
            if len(self.xls_pos) == 1:
                self.xls_capital = self.xls_capital - self.xls_pos[-1][1].price * self.xls_pos[-1][1].volume
            else:
                if self.xls_pos[-1][1].direction == Direction.SHORT:
                    self.xls_capital = self.xls_capital + (self.xls_pos[-1][1].price - self.xls_pos[0][1].price) * \
                                       self.xls_pos[-1][1].volume * self.size \
                                       + self.xls_pos[-1][1].volume * self.xls_pos[0][1].price
                else:
                    self.xls_capital = self.xls_capital + (self.xls_pos[0][1].price - self.xls_pos[-1][1].price) * \
                                       self.xls_pos[-1][1].volume * self.size \
                                       + self.xls_pos[-1][1].volume * self.xls_pos[0][1].price
            if self.xls_pos[-1][0] == 0:
                self.xls_pos = []
    ...
    def cross_limit_order(self):
        ...
        for order in list(self.active_limit_orders.values()):
            ...
            self.xls_pos.append((self.strategy.pos, trade)) # xls 添加
