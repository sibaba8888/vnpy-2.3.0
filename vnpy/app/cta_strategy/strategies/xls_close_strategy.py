...
class XlsCloseStrategy(CtaTemplate):
    """"""
    author = "xls"
    ...
    def on_5min_bar(self, bar: BarData):
        ...
        position_judge_long = np.all([barh_position < self.dis_ma_close * 2, bar30_position < self.dis_ma_close * 2, bar15_position < self.dis_ma_close,
                                      bar5_position < self.dis_ma_close])
        position_judge_short = np.all([barh_position > -self.dis_ma_close * 2, bar30_position > -self.dis_ma_close * 2, bar15_position > -self.dis_ma_close,
                                       bar5_position > -self.dis_ma_close])
        ...
        dis_judge = np.all(
            [dis_h < self.ma_distance * 2, dis_30min < self.ma_distance * 2, dis_15min < self.ma_distance, dis_5min < self.ma_distance])

        trade_judge_long = bar.close_price > np.mean(self.am5.close_array[-6:-2]) + self.break_value and \
                           np.all(trend > 0) and np.all(position > 0) and position_judge_long and dis_judge and \
                           sum((self.emah_long[-6:-1] - self.emah_long[-5])<0)>1
        trade_judge_short = bar.close_price < np.mean(self.am5.close_array[-6:-2]) - self.break_value and \
                            np.all(trend < 0) and np.all(position < 0) and position_judge_short and dis_judge and  \
                            sum((self.emah_long[-6:-1] - self.emah_long[-5])> 0)>1
        #
        # slop_mad = np.mean(
        #     (self.emad_long[-self.slope_period:-1] - self.emad_long[-self.slope_period - 1:-2]) / np.mean(
        #         self.emad_long[-self.slope_period:-1])) * 100
        slop_mad = (self.emad_long[-1] - self.emad_long[-2])/bar.close_price * 100
        if self.pos == 0:
            # if bar.datetime.strftime('%Y-%m-%d') in  ['2017-05-18','2019-05-09','2019-06-05','2019-12-08','2019-12-19']:
            #     print(bar.close_price)
            if trade_judge_long:
                if (slop_mad> -self.slope or slop_mad< -self.slope*2) :
                    self.buy(bar.close_price+20, 2)
        ...
        if self.pos > 0:
            ...
            self.move_max_min_close = max(bar.close_price, self.move_max_min_close)
            ...
            if self.open_method == 'open':
                if (self.max_min_close - self.open[-1]) / self.max_min_close > move_profit / 2:
                    if bar.close_price < self.max_min_close - (self.max_min_close - self.open[0]) * self.draw_back:
                        if 3 not in self.pos_change and self.pos > 1:
                            self.sell(bar.close_price - 20, 2)
                            self.pos_change.append(3)
                            self.move_max_min_close = 0


                if abs(self.pos) <2 and 1 in self.pos_change:
                    if (self.open[0] - bar.close_price) / bar.close_price > self.stop_loss or \
                            ((bar.close_price- self.open[0])/bar.close_price>self.stop_profit and self.emad_long[-1]<self.emad_long[-2]):
                        self.sell(bar.close_price - 20, self.pos)

                if self.emah_long[-1] < self.emah_long[-2]:
                    if abs(slop_mad) < self.slope:
                        if price_open and self.ema15_long[-1] > self.ema15_long[-2]<self.emah_long[-3] and (self.open[-1] - bar.close_price) / bar.close_price > self.stop_loss:
                            self.buy(bar.close_price + 20, 1)
                            self.open_method = 'loss_add'
                            self.move_max_min_close = bar.close_price
                            self.pos_change = []

                if (self.open[0] - bar.close_price) / bar.close_price > self.stop_loss*2 and np.all(trend_d<0) and np.all(bard_position<0) :  # stop loss
                    self.sell(bar.close_price - 20, self.pos)
                    # self.short(bar.close_price +5 ,2)
                    # self.buy_method='open'
                    # self.pos_change = []
                    # self.max_min_close = bar.close_price

                if self.ema15_long[-3] > self.ema15_long[-2] < self.ema15_long[-1] and price_open and bard_position[0][0]<self.dis_ma_close\
                        and slop_mad>-self.slope:
                    if self.pos < 2:
                        self.buy(bar.close_price+5, 1)
                        self.open_method = 'long_add'
                        self.move_max_min_close = bar.close_price

                if slop_mad > self.slope and trend_d[0][0]>0 and bard_position[0][0] < self.dis_ma_close*2 and \
                        np.all(dis_d<self.ma_distance*2) and trade_judge_long:
                    if self.pos < 4:
                        if sum((self.emah_long[-10:-2] - self.emah_long[-9:-1])<0)>5:
                            self.buy(bar.close_price+5, 4-self.pos)
                            self.open_method = 'all'

            elif self.open_method == 'loss_add':
                if (self.move_max_min_close - self.open[-1]) / self.max_min_close > move_profit / 2:
                    if bar.close_price < self.max_min_close - (self.max_min_close - self.open[-1]) * self.draw_back:
                        if 1 not in self.pos_change and self.pos > 1:
                            reduce_pos = max(1, int(self.pos / 2))
                            self.sell(bar.close_price - 20, reduce_pos)
                            self.pos_change.append(1)
                if (self.open[-1] - bar.close_price) / bar.close_price > self.stop_loss and np.all(trend_d<0) :  # stop loss
                    self.sell(bar.close_price - 20, self.pos)

                if self.ema15_long[-3] > self.ema15_long[-2] < self.ema15_long[-1] and price_open and bard_position[0][0] < self.dis_ma_close and \
                    bard_position[0][0]>0:
                    if self.pos < 3:
                        self.buy(bar.close_price + 5, 1)
                        self.open_method = 'long_add'
                        self.pos_change.remove(3)
                        self.move_max_min_close = bar.close_price
                if slop_mad > self.slope and trend_d[0][0]>0 and bard_position[0][0] < self.dis_ma_close*2 and \
                        np.all(dis_d<self.ma_distance*2) and trade_judge_long:
                    if self.pos < 4:
                        if sum((self.emah_long[-10:-2] - self.emah_long[-9:-1])<0)>5:
                            ...
                            self.open_method = 'all'
                            self.pos_change=[]
            elif self.open_method == 'long_add':
                if slop_mad<self.slope:
                    if (self.move_max_min_close - self.open[-1]) / self.move_max_min_close > move_profit / 4:
                        if bar.close_price < self.move_max_min_close - (self.move_max_min_close - self.open[-1]) * self.draw_back and bar.close_price < self.ema15_middle[-1]:
                            if 3 not in self.pos_change and self.pos > 1:
                    ...
                    if (self.move_max_min_close - self.open[-1]) / self.move_max_min_close > move_profit / 4:
                         if bar.close_price<self.emah_long[-1]<self.emah_long[-2]:
                            if 3 not in self.pos_change and self.pos > 1:
                                self.sell(bar.close_price - 20, 1)
                                self.pos_change.append(3)
                                self.move_max_min_close = 0
                if slop_mad > self.slope and trend_d[0][0]>0 and bard_position[0][0] < self.dis_ma_close*2 and \
                        np.all(dis_d<self.ma_distance*2) and trade_judge_long:
                    if self.pos < 4:
                        if sum((self.emah_long[-10:-1] - self.emah_long[-9:])<0)>5:
                            self.buy(bar.close_price+5, 4-self.pos)
                            self.open_method = 'all'
                if self.ema15_long[-3] > self.ema15_long[-2] < self.ema15_long[-1] and price_open and bard_position[0][0] < self.dis_ma_close:
                    if self.pos < 2:
                ...
                if (self.open[0] - bar.close_price) / bar.close_price > self.stop_loss*2 and np.all(trend_d<0) :  # stop loss
                    self.sell(bar.close_price - 20, self.pos)
                    # self.short(bar.close_price - 20, 2)
                    # self.buy_method = 'open'
                    # self.pos_change = []
                    # self.max_min_close = bar.close_price

                if np.all(trend_d<0) and np.all(bard_position<0):
                    self.sell(bar.close_price-20, self.pos)
                    # self.short(bar.close_price-20, 2)
                    # self.buy_method ='open'
                    # self.pos_change=[]
                ...
                if  self.pos <2 and 3 in self.pos_change:
                    if (self.open[0] - bar.close_price) / bar.close_price > self.stop_loss or \
                            ((bar.close_price- self.open[-1])/bar.close_price>self.stop_profit and self.emad_long[-1]<self.emad_long[-2]):
                        self.sell(bar.close_price - 20, self.pos)


            elif self.open_method == 'all':
                if (self.xls_pos[0][1].price - bar.close_price)/bar.close_price > self.stop_loss:   #stop loss
                    self.sell(bar.close_price-20, self.pos)

                elif (bar.close_price - self.xls_pos[0][1].price)/bar.close_price > self.stop_profit:   #stop profit
                    self.sell(bar.close_price-20, int(self.pos)/2)

                elif (self.ema30_long[-3] < self.ema30_long[-2] and self.ema30_long[-2]>self.ema30_long[-1]) and (bar.close_price - self.emad_middle[-1])/bar.close_price>self.shock_area:
                    if 1 not in self.pos_change:
                        tradepos = max(1, int(self.pos/4))
                        self.sell(bar.close_price-5, tradepos)
                        self.pos_change.append(1)

                elif (self.emah_long[-3] < self.emah_long[-2] and self.emah_long[-2]>self.emah_long[-1]) and (bar.close_price - self.emad_long[-1])/bar.close_price>self.shock_area:
                    if 2 not in self.pos_change:
                        tradepos = max(1, int(self.pos/4))
                        self.sell(bar.close_price-5, tradepos)
                        self.pos_change.append(2)

                elif self.emad_long[-3] < self.emad_long[-2] and self.emad_long[-2] > self.emad_long[-1] and np.all(trend_15min<0) and bard_position[0][0]<0:
                    self.sell(bar.close_price-5, self.pos)

                elif (self.ema30_long[-3] > self.ema30_long[-2] and self.ema30_long[-2] < self.ema30_long[-1]) and \
                        (((bar.close_price - self.emad_middle[-1])/bar.close_price<self.dis_ma_close) or ((bar.close_price - self.emad_long[-1])/bar.close_price<self.dis_ma_close)):
                    if self.pos < self.maxpos:
                        self.buy(bar.close_price+4, 2)
        # if self.pos < 0:
        #     self.max_min_close = min(bar.close_price, self.max_min_close)
        #     self.move_max_min_close = min(bar.close_price, self.move_max_min_close)
        #
        #     move_profit = (max(self.amd.close_array[-10:]) - min(self.amd.close_array[-10:])) / bar.close_price
        #     # if bar.datetime.time() == time(14,55):
        #     #     print(time)
        #     if self.open_method == 'open':
        #         if (self.open[-1] - self.max_min_close) / self.max_min_close > move_profit / 2:
        #             if bar.close_price > self.max_min_close + (self.open[-1] - self.max_min_close) * self.draw_back:
        #                 if 1 not in self.pos_change and abs(self.pos) > 1:
        #                     reduce_pos = max(1, abs(int(self.pos / 2)))
        #                     self.cover(bar.close_price + 20, reduce_pos)
        #                     self.pos_change.append(1)
        #
        #         if abs(self.pos) <2 and 1 in self.pos_change:
        #             if (bar.close_price - self.open[0]) / bar.close_price > self.stop_loss or \
        #                 ((self.open[-1] - bar.close_price)/bar.close_price>self.stop_profit and self.emad_long[-1]>self.emad_long[-2]):
        #                 self.cover(bar.close_price + 20, abs(self.pos))
        #
        #
        #         if self.emah_long[-1] > self.emah_long[-2]:
        #             if abs(slop_mad) < self.slope:
        #                 if price_open and self.ema15_long[-1] > self.ema15_long[-2] < self.emah_long[-3] and (
        #                         bar.close_price -self.open[-1]) / bar.close_price > self.stop_loss:
        #                     self.short(bar.close_price - 20, 1)
        #                     self.open_method = 'loss_add'
        #                     self.move_max_min_close = bar.close_price
        #                     self.pos_change = []
        #
        #         if (bar.close_price - self.open[0]) / bar.close_price > self.stop_loss * 2 and np.all(
        #                 trend_d > 0):  # stop loss
        #             self.cover(bar.close_price + 20, abs(self.pos))
        #             self.buy(bar.close_price + 20, 2)
        #             self.buy_method = 'open'
        #             self.pos_change = []
        #             self.max_min_close = bar.close_price
        #         if self.ema15_long[-3] < self.ema15_long[-2] > self.ema15_long[-1] and price_close and bard_position[0][
        #             0] > -self.dis_ma_close:
        #             if abs(self.pos) < 2:
        ...
        #
        #         if slop_mad < -self.slope and trend_d[0][0] <0 and bard_position[0][0] > -self.dis_ma_close * 2 and \
        #                 np.all(dis_d< self.ma_distance*2) and trade_judge_short:
        #             if abs(self.pos) < 4:
        #                 if sum((self.emah_long[-10:-2] - self.emah_long[-9:-1])<0) > 5:
        #                     self.short(bar.close_price + 5, 4 - abs(self.pos))
        #                     self.open_method = 'all'
        #
        #     elif self.open_method == 'loss_add':
        #
        ...
        #                     self.cover(bar.close_price + 20, reduce_pos)
        #                     self.pos_change.append(1)
        #         if (bar.close_price - self.open[-1]) / bar.close_price > self.stop_loss and np.all(trend_d > 0):  # stop loss
        #             self.cover(bar.close_price + 20, abs(self.pos))
        #         if self.ema15_long[-3] < self.ema15_long[-2] > self.ema15_long[-1] and price_close and bard_position[0][
        #             0] > -self.dis_ma_close:
        #             if abs(self.pos) < 2:
        ...
        #         if slop_mad > -self.slope:
        #
        #             if (self.open[-1] - self.move_max_min_close) / self.move_max_min_close > move_profit / 4:
        #                 if bar.close_price > self.move_max_min_close + (self.open[-1]-self.move_max_min_close) * self.draw_back:
        #                     if 3 not in self.pos_change and abs(self.pos) > 1:
        ...
        #                         self.move_max_min_close = bar.close_price
        #
        #         if self.ema15_long[-3] < self.ema15_long[-2] > self.ema15_long[-1] and price_close and bard_position[0][0] < self.dis_ma_close:
        #             if abs(self.pos) < 2:
        #                 self.short(bar.close_price - 5, 1)
        #                 self.open_method = 'long_add'
        #                 self.pos_change.remove(3)
        #                 self.move_max_min_close = bar.close_price
        #         if (bar.close_price - self.open[-1]) / bar.close_price > self.stop_loss * 2 and np.all(
        #                 trend_d > 0):  # stop loss
        #             self.cover(bar.close_price + 20, abs(self.pos))
        #             self.buy(bar.close_price + 20, 2)
        #             self.buy_method = 'open'
        #             self.pos_change = []
        #             self.max_min_close = bar.close_price
        #         if abs(self.pos)<2 and 3 in self.pos_change:
        #             if (bar.close_price - self.open[0]) / bar.close_price > self.stop_loss or \
        #                 ((self.open[0] - bar.close_price)/bar.close_price>self.stop_profit and self.emad_long[-1]>self.emad_long[-2]):
        #                 self.cover(bar.close_price + 20, abs(self.pos))
        #
        #         if slop_mad < -self.slope and trend_d[0][0] <0 and bard_position[0][0] > -self.dis_ma_close * 2 and \
        #                 np.all(dis_d< self.ma_distance*2) and trade_judge_short:
        ...
        #     elif self.open_method == 'all':
        #         if trend_d[0][0] > 0:
        #             self.cover(bar.close_price + 20, self.pos)
