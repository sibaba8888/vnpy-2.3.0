...
class XlsMultiStrategy(CtaTemplate):
    """"""
    author = "xls"
    ...
    middle_period = 60
    short_period = 10
    ma_distance = 0.012     #  the distance of 3 ma in period
    dis_ma_close = 0.012    # the close price distance with ma
    stop_loss = 0.02
    ...
    parameters = ["long_period", "middle_period",
                  ...
                  "dis_ma_close", "dis_ma_close", "stop_loss"]
    slope = 0.1
    ...
    def on_5min_bar(self, bar: BarData):
        ...
        trend = np.concatenate((trend_d, trend_h, trend_30min, trend_5min), axis=0)
        ...
        position = np.concatenate((bard_position, barh_position, bar30_position, bar5_position), axis=0)
        position_judge_long = np.all([bard_position < self.dis_ma_close * 2, barh_position < self.dis_ma_close * 2, bar30_position < self.dis_ma_close * 2,
                                      bar5_position < self.dis_ma_close])
        position_judge_short = np.all([bard_position > -self.dis_ma_close * 2, barh_position > -self.dis_ma_close * 2, bar30_position > -self.dis_ma_close * 2,
                                       bar5_position > -self.dis_ma_close])
        ...
        dis_judge = np.all([dis_d < self.ma_distance * 2, dis_h < self.ma_distance * 2, dis_30min < self.ma_distance * 2, dis_5min < self.ma_distance])

        trade_judge_long = price_open and np.all(position > 0) and position_judge_long and dis_judge and np.all(trend > 0)
        trade_judge_short = price_short and np.all(position < 0) and position_judge_short and dis_judge and np.all(trend<0 )
        ...
        if self.pos == 0:
            ...
            if trade_judge_long and grams>0:
                self.buy(bar.close_price+5,2)
                self.open_method = 'open'
            if trade_judge_short and grams<0:
